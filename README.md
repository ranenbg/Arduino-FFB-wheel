...
If you used some of the earlier firmware versions prior to fw-v22X, windows remembered the axis HID calibration which was +-32k. This issues occurs when you upload lattest firmware with new X-axis calibration 0-65k, which is incompatible with previous settings that windows remembered for the same joystick device Arduino. However, there is a very easy fix for it, all we need to do is to reset the device calibration in windows. This can be done by using program DXtweak, just open it and select Arduino Leonardo as a device (if you have more than 1 ffb capable device), click on device defaults button, then click apply button and close DXtweak. That's it.
