...
void loop() {
  ...
  // milos, added - since we only have 16 pins available for buttons, we will only send 2 bytes
  wheelState[0] = ~(((PINB & B00000010) << 1) | (PIND & B11111011)); // take bit 1 from PORTB + the rest from PORTD B11111x11
  wheelState[1] = ~(((PINB & B00001000) << 3) | ((PINB & B00000001) << 7) | (PINC & B00111111)); // take bit 3 from PORTB and take bit 0 from PORTB + bits 0-5 from PORTC
}
