...
/*! 
	...
	updated: Milos Rankovic (ranenbg@gmail.com), 09.03.2020.
	 - added power down mode selection
*/
/**************************************************************************/
#if ARDUINO >= 100
 #include "Arduino.h"
...
/*! 
    ...
    @param[mode] select modes: 0 - normal, while the rest are for power down
	             1 - 1KOhm to GND, 2 - 100KOhm to GND, 500KOhm to GND
*/
/**************************************************************************/
void Adafruit_MCP4725::setVoltage( uint16_t output, bool writeEEPROM, uint8_t mode )
{
#ifdef TWBR
  ...
  if (mode == 0) {
    if (writeEEPROM) {
      Wire.write(CMD_WRITEDACEEPROM);
    } else {
      Wire.write(CMD_WRITEDAC);
    }
  Wire.write(output >> 4);                   // Upper data bits          (D11.D10.D9.D8.D7.D6.D5.D4)
  Wire.write(output << 4);                   // Lower data bits          (D3.D2.D1.D0.x.x.x.x)
  } else {
    if (mode == 1) {
	  Wire.write(PWD_1KOHM);
    } else if (mode == 2) {
	  Wire.write(PWD_100KOHM);
    } else if (mode == 3) {
	  Wire.write(PWD_500KOHM);
    }
	Wire.write(DAT_ZERO);
	Wire.write(DAT_ZERO);
  } 
